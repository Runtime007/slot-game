const Slot = function (canvas) {
  ...
  this.spin = function () {
    ...
    conf.add.style.filter = 'invert(60%) sepia(65%) saturate(1320%) hue-rotate(342deg) brightness(100%) contrast(97%)';
    conf.add.style.pointerEvents = "none";
    conf.minus.style.filter = 'invert(60%) sepia(65%) saturate(1320%) hue-rotate(342deg) brightness(100%) contrast(97%)';
    conf.minus.style.pointerEvents = "none";
    conf.spinAni.style.animation= 'rotate 0.5s linear infinite';
    conf.win.classList.remove('blink');
  };
    ...
    reels.forEach((reel, i) => {
      ...
      if (delta > conf.reel.animTimes[i] && reel.spinning) {
        ...
        if (i === 2) {
          ...
          conf.add.style.filter = 'invert(5%) sepia(65%) saturate(1320%) hue-rotate(342deg) brightness(100%) contrast(97%)';
          conf.add.style.pointerEvents = "all";
          conf.minus.style.filter = 'invert(5%) sepia(65%) saturate(1320%) hue-rotate(342deg) brightness(100%) contrast(97%)';
          conf.minus.style.pointerEvents = "all";
          conf.spinAni.style.animation= 'rotate 1s linear infinite';
          reel.done(() => {
  ...
  let check = function (reels) {
    ...
    for (let r = 0; r < reels.length; r++) {
      ...
      if (reel1.stop === 0) {
        ...
        if (sum.top !== 0) {
          highlightPts.push(0 + conf.reel.stokeLineHeight / 2);
        }
      }
      //middle line
      if (reel1.stop === 180) {
        ...
        if (sum.middle !== 0) {
          highlightPts.push(120 + conf.reel.stokeLineHeight / 2);
        }
      }
      //bottom line
      if (reel1.stop === 360) {
        ...
        if (sum.bottom !== 0) {
          highlightPts.push(240 + conf.reel.stokeLineHeight / 2);
        }
      }
    }
  ...
  this.autoToggle = function () {
    ...
    if (auto) {
      conf.autoBtn.style.backgroundImage='url(/img/stop.png)'
      conf.autoBackground.style.backgroundImage='url(/img/off.png)'
    } else {
      conf.autoBtn.style.backgroundImage='url(/img/play.png)'
      conf.autoBackground.style.backgroundImage='url(/img/auto.png)'
    }
    conf.spinBtn.click();
  };

  this.add = function () {
    // 500 250 50 25 15 5 2.5 1.5 1 
    if (conf.bet.value * 1 == 1) {
      conf.bet.value = 1.5
    } else if (conf.bet.value * 1 == 1.5) {
      conf.bet.value = 2.5
    } else if (conf.bet.value * 1 == 2.5) {
      conf.bet.value = 5
    } else if (conf.bet.value * 1 == 5) {
      conf.bet.value = 15
    } else if (conf.bet.value * 1 == 15) {
      conf.bet.value = 25
    } else if (conf.bet.value * 1 == 25) {
      conf.bet.value = 50
    } else if (conf.bet.value * 1 == 50) {
      conf.bet.value = 250
    } else if (conf.bet.value * 1 == 250) {
      conf.bet.value = 500
    } else if (conf.bet.value * 1 == 500) {
      alert('Maximum Bet!');
    }
  };
